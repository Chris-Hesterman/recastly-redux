var data1 = [
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/2VqoCa_ZPVw8lUUFMd_WucEkblQ"',
    id: {
      kind: 'youtube#video',
      videoId: 'EfHxUaJDEn8'
    },
    snippet: {
      publishedAt: '2020-03-31T17:00:04.000Z',
      channelId: 'UCVpankR4HtoAVtYnFDUieYA',
      title: 'True Facts: The Wacky Giraffe',
      description:
        'More info on Wildlife ACT: https://www.givengain.com/cc/covid19-keep-wildlifeactive/ My patreon: https://www.patreon.com/truefacts camel = giraffe (oops) ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/EfHxUaJDEn8/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/EfHxUaJDEn8/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/EfHxUaJDEn8/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'zefrank1',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/6yeNi2HS3lP43MqduGoNULScmkY"',
    id: {
      kind: 'youtube#video',
      videoId: 'P_ckAbOr0r4'
    },
    snippet: {
      publishedAt: '2018-02-09T13:00:05.000Z',
      channelId: 'UCDPk9MG2RexnOMGTD-YnSnA',
      title: 'Giraffes 101 | Nat Geo Wild',
      description:
        'How fast do baby giraffes grow? How many vertebrae are in that long neck? A truly unique species, giraffes are found only in sub-Saharan Africa and can reach ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/P_ckAbOr0r4/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/P_ckAbOr0r4/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/P_ckAbOr0r4/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Nat Geo WILD',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/VNKwzIeFV2SLadyIJ3DOcB1WXHo"',
    id: {
      kind: 'youtube#video',
      videoId: 'DXKe2Kpymw0'
    },
    snippet: {
      publishedAt: '2017-12-01T18:09:20.000Z',
      channelId: 'UCBL02YSIvjHwsOey8DmQxIQ',
      title: 'Giraffes - Wild Africa | Giraffe Behaviour and Lifestyle Habitat',
      description:
        "Everyone loves giraffes, but what do we really know about them? It's ironic that the life of one of the world's most identifiable and popular wild animals the giraffe, ...",
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/DXKe2Kpymw0/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/DXKe2Kpymw0/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/DXKe2Kpymw0/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Wildlife Documentary Channel',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/cxsZL-L4dbiXRJH7W8Gr9q5N3_o"',
    id: {
      kind: 'youtube#video',
      videoId: 'uGKdMpZsooE'
    },
    snippet: {
      publishedAt: '2017-09-16T14:00:04.000Z',
      channelId: 'UCWlIGsKGsvgFJHkiZml4O1A',
      title: 'Crazy Facts You Didn&#39;t know About Giraffes',
      description:
        "If you're new, Subscribe! → http://bit.ly/Subscribe-to-Grunge If look at a giraffe long enough, you'll start to suspect there's no way it can actually exist. With those ...",
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/uGKdMpZsooE/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/uGKdMpZsooE/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/uGKdMpZsooE/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Grunge',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/A3EAtbda1wTqSjVAyQ27RMeMaKA"',
    id: {
      kind: 'youtube#video',
      videoId: '7Qw9FZ8ZlDk'
    },
    snippet: {
      publishedAt: '2018-06-15T11:59:56.000Z',
      channelId: 'UCfPyVJEBD7Di1YYjTdS2v8g',
      title:
        'Giraffe Facts for Kids | Classroom Edition Giraffes Learning Video',
      description:
        'Giraffe facts! Learn all about giraffes with these giraffe facts for kids! This is an animal learning video you will not want to miss! Perfect for the classroom and ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/7Qw9FZ8ZlDk/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/7Qw9FZ8ZlDk/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/7Qw9FZ8ZlDk/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Homeschool Pop',
      liveBroadcastContent: 'none'
    }
  }
];

var data2 = [
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/ytGBEHhZd5Ico-uLkZilNqJh21Y"',
    id: {
      kind: 'youtube#video',
      videoId: 'TgppkTXQMtc'
    },
    snippet: {
      publishedAt: '2020-04-03T14:00:11.000Z',
      channelId: 'UCwcsDWGip6vtiZyCnvDoClQ',
      title: 'Cats vs Invisible Wall (ft. Chicken)',
      description:
        'Kitties attacked on the middle of peaceful chicken dinner. Could the kitties break the invisible wall again?',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/TgppkTXQMtc/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/TgppkTXQMtc/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/TgppkTXQMtc/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Kittisaurus',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/t5noQZJFHDmt9JWc00K1VTOKDcA"',
    id: {
      kind: 'youtube#video',
      videoId: 'tg-6lz50-Tw'
    },
    snippet: {
      publishedAt: '2020-04-03T08:04:20.000Z',
      channelId: 'UCuPLku1Zrk6HMr2S51yGkpQ',
      title: 'Cats being ... CATS 😹~ Tiktok Compilation #4',
      description:
        'Cats being ... CATS ~ Tiktok Compilation #4 --------------------------------------------------------------------------- COPYRIGHT ISSUES: Our purpose, when making ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/tg-6lz50-Tw/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/tg-6lz50-Tw/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/tg-6lz50-Tw/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Funny And Cute Kitten Cat',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/j-VjmqPpZHxtuY14CHFTTyMJPEU"',
    id: {
      kind: 'youtube#video',
      videoId: 'DKxCvAc-DvU'
    },
    snippet: {
      publishedAt: '2020-03-29T19:00:11.000Z',
      channelId: 'UC09IvZwjpunzrdHH1EHok-w',
      title:
        '🤣 Funniest 🐶 Dogs and 😻 Cats - Awesome Funny  Animals Videos 😇',
      description:
        "Enjoy new funniest and very cute compilation of the week about try not laugh funny animals' life video. But some cats in this video will actually surprise you!",
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/DKxCvAc-DvU/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/DKxCvAc-DvU/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/DKxCvAc-DvU/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: "Funny Animals' Life",
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/YY7oY0rFsv-CZ84dARBpYhqB0Tc"',
    id: {
      kind: 'youtube#video',
      videoId: 'NhHBElaarrI'
    },
    snippet: {
      publishedAt: '2020-03-31T12:00:45.000Z',
      channelId: 'UC8hC-augAnujJeprhjI0YkA',
      title:
        'Baby Cats - Cute and Funny Cat Videos Compilation #32 | Aww Animals',
      description:
        'Watching funny baby cats is the hardest try not to laugh challenge. Baby cats are amazing creature because they are the cutest and most funny. This is the cutest ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/NhHBElaarrI/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/NhHBElaarrI/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/NhHBElaarrI/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Aww Animals',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/kBpqOGC8piekyI_dObRJLyDmtho"',
    id: {
      kind: 'youtube#video',
      videoId: 'i09cua9QN7I'
    },
    snippet: {
      publishedAt: '2020-03-25T17:44:35.000Z',
      channelId: 'UC8hC-augAnujJeprhjI0YkA',
      title:
        'Baby Cats - Cute and Funny Cat Videos Compilation #31 | Aww Animals',
      description:
        'Baby cats are amazing creature because they are the cutest and most funny. I challenge anyone watching this video to try not to laugh when you see the funny ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/i09cua9QN7I/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/i09cua9QN7I/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/i09cua9QN7I/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Aww Animals',
      liveBroadcastContent: 'none'
    }
  }
];

var data3 = [
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/hmlUlLW9HRX8p_uoVylT0LpGC08"',
    id: {
      kind: 'youtube#video',
      videoId: '9frLs4XicFY'
    },
    snippet: {
      publishedAt: '2020-04-02T22:16:55.000Z',
      channelId: 'UC5PrkGgI_cIaSStOyRmLAKA',
      title: 'Barstool Pizza Review - Brew Pub Lotzza Motzza',
      description:
        'The Lotzza Motzza people showed up to play with their gifts and Dave may or may not have been influenced/ Download The One Bite App to see more and ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/9frLs4XicFY/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/9frLs4XicFY/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/9frLs4XicFY/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'One Bite Pizza Reviews',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/7fgNIMKJvlNxG-w0MNVxe6XiVjE"',
    id: {
      kind: 'youtube#video',
      videoId: 'smjnOG1diXg'
    },
    snippet: {
      publishedAt: '2020-03-28T16:00:03.000Z',
      channelId: 'UCbpMy0Fg74eXXkvxJrtEn3w',
      title: 'Every Way to Make Pizza (32 Methods) | Bon Appétit',
      description:
        "Is there any other way to make pizza other than boiling it? Wait, there is? Ok, so apparently there's also oven-baked, cast-iron and microwaved pizza. In fact ...",
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/smjnOG1diXg/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/smjnOG1diXg/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/smjnOG1diXg/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Bon Appétit',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/1aGq8Y7e8DMewNTsUgkW_2K83tE"',
    id: {
      kind: 'youtube#video',
      videoId: 'MpQxZVQPK8k'
    },
    snippet: {
      publishedAt: '2020-04-03T14:15:02.000Z',
      channelId: 'UCRbXS1MKrZ55Vm5G9336BKw',
      title:
        'PIZZA CASEIRA ECONÔMICA DE LIQUIDIFICADOR | SIMPLES E PRONTA EM 30 MINUTOS | RECEITAS DA CRIS',
      description:
        'E a receita de hoje é um deliciosa pizza caseira de liquidificador com massa incrível. Pronta em 30 minutinhos !!! Então, bora se animar, anotar os ingredientes ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/MpQxZVQPK8k/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/MpQxZVQPK8k/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/MpQxZVQPK8k/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Receitas Da Cris',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/JJsUk92LeTdyKEqQXlJ4e0IdMWM"',
    id: {
      kind: 'youtube#video',
      videoId: 'KPdDm0mo2Ck'
    },
    snippet: {
      publishedAt: '2020-04-01T22:00:06.000Z',
      channelId: 'UC5PrkGgI_cIaSStOyRmLAKA',
      title: 'Barstool Frozen Pizza Review - California Pizza Kitchen',
      description:
        "The packages are starting to stack up quickly in Daves apartment, but before he opens those he reviews California Pizza Kitchen's frozen cheese pizza.",
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/KPdDm0mo2Ck/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/KPdDm0mo2Ck/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/KPdDm0mo2Ck/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'One Bite Pizza Reviews',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/L85K5I7xxDHRwLwjJ6gswDnnKhQ"',
    id: {
      kind: 'youtube#video',
      videoId: '5PV-dUZ8jKY'
    },
    snippet: {
      publishedAt: '2020-04-02T09:00:14.000Z',
      channelId: 'UCxCKl-K0sedNZp7xfWg0VTw',
      title: 'Samurai ไปไหนวะ - 35 - Pizza ร้านแรกของอเมริกา...ไปดู',
      description:
        'ดูคลิปก่อนใครได้ที่ : https://www.twitch.tv/videos/576631218 ติดต่องานรับทราบได้ที่ EMAIL : rubsarbproduction@gma...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/5PV-dUZ8jKY/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/5PV-dUZ8jKY/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/5PV-dUZ8jKY/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'RUBSARB production',
      liveBroadcastContent: 'none'
    }
  }
];

var data4 = [
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/E_V6RZYpIX8yBcgFyUe2qsDEy-0"',
    id: {
      kind: 'youtube#video',
      videoId: 'fBYvHHT8fdE'
    },
    snippet: {
      publishedAt: '2020-03-27T07:24:28.000Z',
      channelId: 'UCAmhbG40GSFEJEa-6Yj8zAQ',
      title: '♥Cute Puppies Doing Funny Things 2020♥ #3 Cutest Dogs',
      description:
        'Cute Puppies Doing Funny Things 2020♥ #3 Cutest Dogs Thanks For Watching ! Please Like Share & Comment If You Like This Video ! Subscribe CUTE ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/fBYvHHT8fdE/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/fBYvHHT8fdE/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/fBYvHHT8fdE/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Cute Babies and Pets TV',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/gNsmRX5iJkf0RGoe0X2FaEzAnpk"',
    id: {
      kind: 'youtube#video',
      videoId: 'wtH-hdOF1uA'
    },
    snippet: {
      publishedAt: '2020-02-08T15:32:50.000Z',
      channelId: 'UC8hC-augAnujJeprhjI0YkA',
      title:
        'Baby Dogs - Cute and Funny Dog Videos Compilation #20 | Aww Animals',
      description:
        'Watching funny baby dogs is the hardest try not to laugh challenge. Baby dogs are amazing creature because they are the cutest and most funny. This is the ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/wtH-hdOF1uA/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/wtH-hdOF1uA/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/wtH-hdOF1uA/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Aww Animals',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/USgSAF0IapM9k6Q80pejKjTGoyI"',
    id: {
      kind: 'youtube#video',
      videoId: 'BkD2nN5275c'
    },
    snippet: {
      publishedAt: '2020-02-19T20:53:36.000Z',
      channelId: 'UC09IvZwjpunzrdHH1EHok-w',
      title:
        '🤣 Funniest 🐶 Dogs and 😻 Cats - Awesome Funny Pet Animals Videos 😇',
      description:
        "Enjoy new funniest and very cute compilation of the week about try not laugh funny animals' life video. But some cats in this video will actually surprise you!",
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/BkD2nN5275c/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/BkD2nN5275c/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/BkD2nN5275c/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: "Funny Animals' Life",
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/dkVsFonSTHSsVgcan59qRr-UCko"',
    id: {
      kind: 'youtube#video',
      videoId: 'NwlwJjelgzs'
    },
    snippet: {
      publishedAt: '2018-03-30T11:30:00.000Z',
      channelId: 'UCKy3MG7_If9KlVuvw3rPMfw',
      title: 'Get ready for LAUGHING SUPER HARD - Best FUNNY DOG videos',
      description:
        'Dogs and puppies are super funny and hilarious, they make us laugh all the time! The hardest TRY NOT TO LAUGH challenge in the World! Just look how all ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/NwlwJjelgzs/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/NwlwJjelgzs/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/NwlwJjelgzs/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Tiger Productions',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/2QgI1vTASchbAeuImetEomJdGp0"',
    id: {
      kind: 'youtube#video',
      videoId: 'lyelxGIydeQ'
    },
    snippet: {
      publishedAt: '2020-01-13T14:07:51.000Z',
      channelId: 'UCvznoyf6e8T1NRHLkfxwG9Q',
      title:
        'Cute Dogs and Babies are Best Friends - Dogs Babysitting Babies Video',
      description:
        'Video: Cute Dogs and Babies are Best Friends - Dogs Babysitting Babies Video ☞ Link video: https://youtu.be/lyelxGIydeQ Thanks for watching! Wait to see ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/lyelxGIydeQ/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/lyelxGIydeQ/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/lyelxGIydeQ/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Baby Awesome',
      liveBroadcastContent: 'none'
    }
  }
];

var data5 = [
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/obMmYrWWrGT5YhDDS8gaEPQMwTc"',
    id: {
      kind: 'youtube#video',
      videoId: 'OmDAIeWDqt0'
    },
    snippet: {
      publishedAt: '2020-04-02T19:00:04.000Z',
      channelId: 'UCXuqSBlHAE6Xw-yeJA0Tunw',
      title: 'I threw a party and no one came...',
      description:
        'Thanks to Soundcore by Anker for sponsoring the video! Sign up to win prizes and Flare 2 discounts in the #100Flare2 Giveaway: https://s.krpax.com/r/579N2u ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/OmDAIeWDqt0/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/OmDAIeWDqt0/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/OmDAIeWDqt0/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Linus Tech Tips',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/2AUCaDneHShZx0hO15K8Uj9iwRM"',
    id: {
      kind: 'youtube#video',
      videoId: 'pYQtusd8deU'
    },
    snippet: {
      publishedAt: '2020-04-01T19:32:53.000Z',
      channelId: 'UCXuqSBlHAE6Xw-yeJA0Tunw',
      title: 'The Truth about LinusTechTips - EXPOSED (April Fools 2020)',
      description:
        "Get 50% off your first 3 months of FreshBooks when you sign up for a paid plan at https://www.freshbooks.com/techtips Get iFixit's NEW Mahi Driver Kit today for ...",
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/pYQtusd8deU/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/pYQtusd8deU/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/pYQtusd8deU/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Linus Tech Tips',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/40qpQywqMsl8FrOsEvFGqkFoQhE"',
    id: {
      kind: 'youtube#video',
      videoId: 'DdzwSM3HAuA'
    },
    snippet: {
      publishedAt: '2020-03-29T19:14:37.000Z',
      channelId: 'UCXuqSBlHAE6Xw-yeJA0Tunw',
      title: 'How BAD is a BestBuy Gaming PC?',
      description:
        "Get a $20 Off Perk on the Massdrop x Sennheiser PC37X Gaming Headset at https://dro.ps/ltt-feb-pc37x Get iFixit's NEW Mahi Driver Kit today for only $29.99 ...",
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/DdzwSM3HAuA/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/DdzwSM3HAuA/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/DdzwSM3HAuA/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Linus Tech Tips',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/r9IhxmLN4J3mDSVeL-cP2C1eqPc"',
    id: {
      kind: 'youtube#video',
      videoId: 'JBwMDfMfSNU'
    },
    snippet: {
      publishedAt: '2020-03-28T19:00:43.000Z',
      channelId: 'UCXuqSBlHAE6Xw-yeJA0Tunw',
      title: 'PewDiePie! - We built you a gaming PC!',
      description:
        "Check out the March Expo 2020 on Alibaba, and get the DOQAUS V9 on Alibaba.com: https://bit.ly/MarchExpo_Linus2 We've secretly been building a PC for ...",
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/JBwMDfMfSNU/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/JBwMDfMfSNU/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/JBwMDfMfSNU/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Linus Tech Tips',
      liveBroadcastContent: 'none'
    }
  },
  {
    kind: 'youtube#searchResult',
    etag: '"xwzn9fn_LczrfK9QS3iZcGzqRGs/UjZTu4RjpNUyoVhf999t6DTY-CQ"',
    id: {
      kind: 'youtube#video',
      videoId: '75L8Hrb49A4'
    },
    snippet: {
      publishedAt: '2019-07-25T19:16:21.000Z',
      channelId: 'UCXuqSBlHAE6Xw-yeJA0Tunw',
      title: 'Cheap vs. Expensive Gaming!?',
      description:
        'Get the Ring Doorbell Welcome Kit today at https://Ring.com/LTT Use code LINUS and get 25% off GlassWire at https://lmg.gg/glasswire We built six gaming PCs ...',
      thumbnails: {
        default: {
          url: 'https://i.ytimg.com/vi/75L8Hrb49A4/default.jpg',
          width: 120,
          height: 90
        },
        medium: {
          url: 'https://i.ytimg.com/vi/75L8Hrb49A4/mqdefault.jpg',
          width: 320,
          height: 180
        },
        high: {
          url: 'https://i.ytimg.com/vi/75L8Hrb49A4/hqdefault.jpg',
          width: 480,
          height: 360
        }
      },
      channelTitle: 'Linus Tech Tips',
      liveBroadcastContent: 'none'
    }
  }
];

const moreData = {
  giraffes: data1,
  cats: data2,
  pizza: data3,
  dogs: data4,
  'linus tech tips': data5
};
export default moreData;
